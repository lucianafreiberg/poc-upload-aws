package com.lucianafreiberg.pocupload;
...
import com.lucianafreiberg.pocupload.controller.UploadResult;
...
class PocUploadApplicationTests {
    ...
    @Test
    void wheUploadSingleFile_thenSuccess() throws Exception {

        String url = "http://Localhost:" + serverPort + "/inbox";
        byte[] data = Files.readAllBytes(Paths.get("src/test/resource/South.jpg"));
        UploadResult result = restTemplate.postForObject(url, data, UploadResult.class);

        assertEquals("Expceted CREATED (202)", result.getStatus(), HttpStatus.CREATED);
    }
    ...
    void whenUploadMultipleFiles_thenSuccess() throws Exception {
        ...
        addFileEntity("f1", body, new File("src/test/resources/South.jpg"));
        addFileEntity("f2", body, new File("src/test/resources/South1.jpg"));
